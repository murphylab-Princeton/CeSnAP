function export_to_excel_threshold(info, filePD, ResultPD)
...
%% ------------------------------------------------------------------------
                        ...
                        for RoundID = 1:length(filePD(CaseID).worms) 
                         ...
                         for SnapID = 1:length(filePD(CaseID).worms{RoundID})
                                for k  = 1:length(filePD(CaseID).worms{RoundID}{SnapID}.worm_bc)
                                            if ResultPD.CurlCrit{CaseID}{RoundID}{SnapID}{k}==1,         SplitRoundCurl{aa}{RoundID}(bb)      = SplitRoundCurl{aa}{RoundID}(bb)     +1;         end
                                            if ResultPD.CurlCrit{CaseID}{RoundID}{SnapID}{k}~=2,         SplitRoundTotal{aa}{RoundID}(bb)     = SplitRoundTotal{aa}{RoundID}(bb)    +1;         end
                                            if ResultPD.tagNearCurlState{CaseID}{RoundID}{SnapID}{k}==1, SplitRoundNearCurl{aa}{RoundID}(bb)  = SplitRoundNearCurl{aa}{RoundID}(bb) +1;         end
                                end
                         end
                         end
                    end
    ...
    NameCellMatrix(1:476,1)       = {''};
    for aa=1:length(info.mConditionName)
        NameCellMatrix((40*(aa-1))+1,1)                      =  {info.mmConditionName{aa}};
    end
    xlswrite('DataOutput.xlsx',NameCellMatrix      ,'Sheet1', 'A1:A476' );
%%-------------------------------------------------------------------------
end
